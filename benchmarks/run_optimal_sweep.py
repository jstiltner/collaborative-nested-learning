...
def run_with_cms(
    ...
    for task_id in range(config.num_tasks):
        ...
        for epoch in range(config.num_epochs):
            task_loss, reg_loss = train_epoch_cms(model, optimizer, train_loader, config.device)
...
def run_optimal_sweep(config: Optional[OptimalSweepConfig] = None) -> Dict[str, Any]:
    ...
    for strength in config.reg_strengths:
        ...
        print(f"    Accuracy: {result['average_accuracy']:.4f}, "
    ...
    for strength in config.reg_strengths:
        r = strength_results[str(strength)]
        print(f"{strength:<12} {r['average_accuracy']:>10.4f} "
    ...
    print("-" * 52)
    print(f"{'CMS + Bridges':<20} {with_bridges['average_accuracy']:>10.4f} "
          f"{with_bridges['forgetting']:>12.4f} {with_bridges['bridge_transfers']:>10}")
    print(f"{'CMS only':<20} {without_bridges['average_accuracy']:>10.4f} "
    ...
    print(f"\nBridge Contribution:")
    print(f"  Accuracy: {acc_diff:+.4f} ({acc_diff/without_bridges['average_accuracy']*100:+.2f}%)")
    print(f"  Forgetting reduction: {forget_diff:+.4f} ({forget_diff/without_bridges['forgetting']*100:+.2f}%)")
