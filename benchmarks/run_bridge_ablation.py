...
def run_bridge_ablation(
    config: Optional[BridgeAblationConfig] = None,
) -> Dict[str, Any]:
    ...
    for strength in config.reg_strengths:
        ...
        print(
            f"    CMS only:    acc={cms_only['average_accuracy']:.4f}, forget={cms_only['forgetting']:.4f}"
        )
        print(
            f"    CMS+Bridges: acc={cms_bridges['average_accuracy']:.4f}, forget={cms_bridges['forgetting']:.4f}"
        )
        print(
            f"    Bridge contribution: acc={acc_diff:+.4f}, forget_reduction={forget_diff:+.4f}"
        )
    ...
    for strength in config.reg_strengths:
        ...
        print(
            f"{strength:<10} {cms['average_accuracy']:>10.4f} {bridges['average_accuracy']:>12.4f} "
    ...
    print(
        f"Bridges reduce forgetting in {bridge_helps_forgetting}/{len(config.reg_strengths)} settings"
    )
