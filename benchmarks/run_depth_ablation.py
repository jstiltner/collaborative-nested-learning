...
def run_depth_experiment(
    ...
    print(f"\n{'='*60}")
    print(f"Running {depth_config.name} (levels={depth_config.num_levels}, "
    ...
    for task_id, train_loader in enumerate(tasks):
        ...
        for epoch in range(epochs_per_task):
            ...
            if epoch == epochs_per_task - 1:
                ...
                if bridge_activity['attempts'] > 0:
                    print(f"    Bridge activity: {bridge_activity['transfers']}/{bridge_activity['attempts']} transfers, "
                          f"avg_gate={bridge_activity['avg_gate']:.3f}, "
                          f"range=[{bridge_activity['min_gate']:.3f}, {bridge_activity['max_gate']:.3f}]")
                elif use_bridges:
                    print(f"    Bridge activity: 0 attempts (bridge_frequency too high)")
        ...
        print(f"    Accuracies after Task {task_id + 1}: {[f'{a:.2%}' for a in task_accuracies]}")
    ...
    if use_bridges:
        ...
        avg_gate = (sum(total_bridge_activity['gate_values']) / len(total_bridge_activity['gate_values'])
                   if total_bridge_activity['gate_values'] else 0.0)
        print(f"\n  Bridge Summary:")
...
def run_full_ablation(
    ...
    for r in all_results:
        ...
        print(f"{r['config']:<15} {bridges_str:<10} {r['avg_accuracy']:.2%}       {r['avg_forgetting']:.2%}")
    
    # Find best configuration
    best_result = max(all_results, key=lambda x: x['avg_accuracy'] - x['avg_forgetting'])
