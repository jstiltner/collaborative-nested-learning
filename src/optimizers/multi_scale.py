...
class MultiScaleBridges(nn.Module):
    ...
    def forward(
        ...
        if len(states) != self.num_levels:
            raise ValueError(f"Expected {self.num_levels} states, got {len(states)}")
...
def _test_multi_scale():
    ...
    for num_levels in [3, 5, 7]:
        ...
        print(
            f"    Learning rates: {[f'{lr:.4f}' for lr in config.get_learning_rates()]}"
        )
        ...
        ), f"Loss should decrease for {num_levels} levels"
